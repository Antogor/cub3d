...
/*   Updated: 2020/02/12 12:11:02 by agarzon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void            my_mlx_pixel_put(t_mlx *data, int x, int start, t_color *color2, int end, int side)
{
	int i = 0;
	int y = 0;
	while(i < start)
	...
	while (i < end)
	{
		if (side == 1)
			...
			*(data->img_data + x + i * data->size_l / 4) = color2->colorW;
		i++;
	}
...
int		main(int argc, char **argv)
{
	...
	color2->colorW = create_trgb(0, 100, 0, 255);
	...
	while(x < screenWidth)
	{
		...
		printf("DRAWEND: %d\n", drawEnd);
		printf("SSSS\n");
		my_mlx_pixel_put(img, x, drawStart, color2, drawEnd, side);
		x++;
		printf("X = %d\n", x);
		printf("\n");
	}
